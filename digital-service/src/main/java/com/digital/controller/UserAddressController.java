...
public class UserAddressController {
     ...
     * @return 默认地址信息
     */
    @GetMapping("/userAddress/default")
    @AuthCheck(mustRole = UserConstant.DEFAULT_ROLE)
    public Result<GetUserAddressVo> getDefaultUserAddress(HttpServletRequest request) {
        ...
        UserAddress defaultAddress = userAddressService.getOne(queryWrapper);

        // 如果用户没有任何地址
        if (defaultAddress == null) {
            return Result.error("用户没有设置地址");
        }

        GetUserAddressVo addressVo = new GetUserAddressVo();
